...
<https://docs.forgefirm.org/technical/forgefirm/forgectrl/>. The cooling
engine has its own, at `.../cooling-engine/`.
...
tests are in `tests/`. `tests/fflog_e2e.sh` proves the whole logging path
against a private rsyslogd. `tests/test_devserver_mock.py` parses the C
tables and holds the panel's dev-server mock to them, so a new settings key,
a new gate row or a new route needs its mock entry in the same change.

The board binary is a cross-build, never a native one:
`forgefirm/scripts/bench/build-forgectrl.sh`.

### Rules specific to this repository

- **This daemon is the single writer of the thermal hardware** in both
  controller modes. Nothing else writes the fans, the pump, the TEC or the
  flow-check heater. The one sanctioned exception is a controller's
  compiled-in fallback when the engine is provably absent.
- **Never poll the Grbl socket on TCP port 23 for machine state.** A
  connection there displaces the sender's session. Position comes from the
  kernel step counters, anchored at the last completed homing. Controller
  facts arrive as pushed state files under `/run/forgefirm`.
- **Every safety, job and timeout decision runs on `CLOCK_MONOTONIC`.** The
  board has no real-time clock, and the wall clock steps by years across a
  cold boot. The wall clock is for display and log stamps only.
...
The acceptance page in `forgefirm` shares this repository's `theme.css` and
its vendored Bootstrap, byte for byte. Land a panel UI change **here**
first, push it, and pin it. Only then push `forgefirm`, or its CI goes red.
